...
import tv.mixiong.xinge.json.JSONArray;
...
public class XingeApp {
	...
	private Logger logger = LoggerFactory.getLogger(XingeApp.class);

	private static final String RESTAPI_PUSHSINGLEDEVICE = "http://openapi.xg.qq.com/v2/push/single_device";
	private static final String RESTAPI_PUSHSINGLEACCOUNT = "http://openapi.xg.qq.com/v2/push/single_account";
	private static final String RESTAPI_PUSHACCOUNTLIST = "http://openapi.xg.qq.com/v2/push/account_list";
	private static final String RESTAPI_PUSHALLDEVICE = "http://openapi.xg.qq.com/v2/push/all_device";
	private static final String RESTAPI_PUSHTAGS = "http://openapi.xg.qq.com/v2/push/tags_device";
	private static final String RESTAPI_QUERYPUSHSTATUS = "http://openapi.xg.qq.com/v2/push/get_msg_status";
	private static final String RESTAPI_QUERYDEVICECOUNT = "http://openapi.xg.qq.com/v2/application/get_app_device_num";
	private static final String RESTAPI_QUERYTAGS = "http://openapi.xg.qq.com/v2/tags/query_app_tags";
	private static final String RESTAPI_CANCELTIMINGPUSH = "http://openapi.xg.qq.com/v2/push/cancel_timing_task";
	private static final String RESTAPI_BATCHSETTAG = "http://openapi.xg.qq.com/v2/tags/batch_set";
	private static final String RESTAPI_BATCHDELTAG = "http://openapi.xg.qq.com/v2/tags/batch_del";
	private static final String RESTAPI_QUERYTOKENTAGS = "http://openapi.xg.qq.com/v2/tags/query_token_tags";
	private static final String RESTAPI_QUERYTAGTOKENNUM = "http://openapi.xg.qq.com/v2/tags/query_tag_token_num";
	private static final String RESTAPI_CREATEMULTIPUSH = "http://openapi.xg.qq.com/v2/push/create_multipush";
	private static final String RESTAPI_PUSHACCOUNTLISTMULTIPLE = "http://openapi.xg.qq.com/v2/push/account_list_multiple";
	private static final String RESTAPI_PUSHDEVICELISTMULTIPLE = "http://openapi.xg.qq.com/v2/push/device_list_multiple";
	private static final String RESTAPI_QUERYINFOOFTOKEN = "http://openapi.xg.qq.com/v2/application/get_app_token_info";
	private static final String RESTAPI_QUERYTOKENSOFACCOUNT = "http://openapi.xg.qq.com/v2/application/get_app_account_tokens";
	private static final String RESTAPI_DELETETOKENOFACCOUNT = "http://openapi.xg.qq.com/v2/application/del_app_account_tokens";
	private static final String RESTAPI_DELETEALLTOKENSOFACCOUNT = "http://openapi.xg.qq.com/v2/application/del_app_account_all_tokens";


	private static final int IOSENV_PROD = 1;
	private static final int IOSENV_DEV = 2;

	private static final long IOS_MIN_ID = 2200000000L;
	...
	// 易用的api接口v1.1.4引入
	public JSONObject pushTokenAndroid(PushParam param)
	{
		Message message = new Message();
		message.setType(Message.TYPE_MESSAGE);
		message.setTitle(param.getTitle());
		message.setContent(param.getContent());
		message.setCustom(ifHaveCustomerConf(param.getActionUrl()));
		JSONObject ret = pushSingleDevice(param.getToken(), message);
		return (ret);
	}

	public  void batchAndroidPushByTokens(List<String> tokens, PushParam param) {
		if (tokens == null || tokens.size() > 1000) {
			return;
		}
		logger.info("batch android push params: " + JSON.toJSONString(param));
		Message message = new Message();
		message.setType(Message.TYPE_MESSAGE);
		message.setTitle(param.getTitle());
		message.setContent(param.getContent());
		message.setCustom(ifHaveCustomerConf(param.getActionUrl()));
		JSONObject result = createMultipush(message);
		logger.info("xinge android batch push response: " + result.toString());
		int retCode = result.getInt("ret_code");
		if (retCode == EventConstants.SUCCESS) {
			PushResponse pushResponse = JSON.parseObject(result.get("result").toString(), PushResponse.class);
			String pushId = pushResponse.getPushId();
			pushDeviceListMultiple(Long.parseLong(pushId), tokens);
		}
	}

	public void batchIOSPushByTokens(List<String> tokens, PushParam param) {
		if (tokens == null || tokens.size() > 1000) {
			return;
		}
		logger.info("batch ios push params: " + param.toString());
		MessageIOS message = new MessageIOS();
		message.setAlert(param.getContent());
		message.setBadge(1);
		message.setSound("beep.wav");
		message.setCustom(ifHaveCustomerConf(param.getActionUrl()));
		JSONObject result = createMultipush(message, param.getEnv());
		logger.info("xinge IOS batch push response: " + result.toString());
		int retCode = result.getInt("ret_code");
		if (retCode == EventConstants.SUCCESS) {
			PushResponse pushResponse = JSON.parseObject(result.get("result").toString(), PushResponse.class);
			String pushId = pushResponse.getPushId();
	...
	public JSONObject pushAccountAndroid(PushParam param)
	{
		Message message = new Message();
		...
		message.setContent(param.getContent());
		message.setCustom(ifHaveCustomerConf(param.getActionUrl()));
		JSONObject ret = pushSingleAccount(0, param.getAccount(), message);
		return (ret);
	}

	public JSONObject pushAllAndroid(PushParam param)
	{
		Message message = new Message();
		message.setType(Message.TYPE_NOTIFICATION);
		message.setTitle(param.getTitle());
	...
	public JSONObject pushTagAndroid(PushParam param)
	{
		Message message = new Message();
		message.setType(Message.TYPE_MESSAGE);
		message.setTitle(param.getTitle());
		message.setContent(param.getContent());
		message.setCustom(ifHaveCustomerConf(param.getActionUrl()));
		if (!StringUtils.isEmpty(param.getSendTime())) {
			message.setSendTime(param.getSendTime());
		}
		List<String> tagList = new ArrayList<String>();
		tagList.add(param.getTag());
		JSONObject ret = pushTags(0, tagList, "OR", message);
		return (ret);
	}

	public JSONObject pushTokenIos(PushParam param)
	{
		MessageIOS message = new MessageIOS();
		message.setAlert(param.getContent());
		message.setBadge(1);
		message.setSound("beep.wav");
		message.setCustom(ifHaveCustomerConf(param.getActionUrl()));
		JSONObject ret = pushSingleDevice(param.getToken(), message, param.getEnv());
		return (ret);
	}

	public JSONObject pushAccountIos(PushParam param)
	{
		MessageIOS message = new MessageIOS();
		message.setAlert(param.getContent());
		message.setBadge(1);
		...
		JSONObject ret = pushSingleAccount(0, param.getAccount(), message, param.getEnv());
		return (ret);
	}

	public JSONObject pushAllIos(PushParam param)
	{
		MessageIOS message = new MessageIOS();
		message.setAlert(param.getContent());
		message.setBadge(1);
		...
		JSONObject ret = pushAllDevice(0, message, param.getEnv());
		return (ret);
	}

	public JSONObject pushTagIos(PushParam param) {
		MessageIOS message = new MessageIOS();
		message.setAlert(param.getContent());
		message.setBadge(1);
		message.setSound("beep.wav");
		...
		List<String> tagList = new ArrayList<String>();
		tagList.add(param.getTag());
		return (pushTags(0, tagList, "OR", message, param.getEnv()));
	}
	...
	public JSONObject pushSingleDevice(String deviceToken, MessageIOS message, int environment)
	{
		...
		if (!StringUtils.isEmpty(message.getSendTime())) {
			...
			params.put("send_time", "1970-01-01 00:00:00");
		}
		params.put("device_token", deviceToken);
		params.put("message_type", message.getType());
		params.put("message", message.toJson());
	...
	protected static Map<String, Object> ifHaveCustomerConf(String actionUrl) {
		Map<String, Object> map = Maps.newHashMap();
		if (!StringUtils.isEmpty(actionUrl)) {
			map.put("action_url", actionUrl);
		}
		return map;
	}
	...
	public XingeApp buildAndroidClient() {
		this.m_accessId = Long.parseLong(xingeProperties.getAndroidAppId());
		this.m_secretKey = xingeProperties.getAndroidSecretKey();
		return this;
	}

	public XingeApp buildIosClient() {
		this.m_accessId = Long.parseLong(xingeProperties.getIosAppId());
		this.m_secretKey = xingeProperties.getIosSecretKey();
		return this;
	}
