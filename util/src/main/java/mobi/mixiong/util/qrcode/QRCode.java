 ...
 * QRコード. <br/>
 * ■使い方 <br/>
 * (1) 誤り訂正レベル、データ等、諸パラメータを設定します。 <br/>
 * (2) make() を呼び出してQRコードを作成します。 <br/>
 * (3) getModuleCount() と isDark() で、QRコードのデータを取得します。 <br/>
 * 
 */
public class QRCode {
	...
	private void make(boolean test, int maskPattern) {
		...
		QRData[] dataArray = (QRData[]) qrDataList
				.toArray(new QRData[qrDataList.size()]);
	...
	private void setupPositionProbePattern(int row, int col) {
		...
		for (int r = -1; r <= 7; r++) {
			...
			for (int c = -1; c <= 7; c++) {
				...
				if (row + r <= -1 || moduleCount <= row + r || col + c <= -1
						|| moduleCount <= col + c) {
					continue;
				}
	...
	public static byte[] createData(int typeNumber, int errorCorrectLevel,
			QRData[] dataArray) {
	...
	private static byte[] createBytes(BitBuffer buffer, RSBlock[] rsBlocks) {
		...
		for (int r = 0; r < rsBlocks.length; r++) {
			...
			Polynomial rawPoly = new Polynomial(dcdata[r],
					rsPoly.getLength() - 1);
	...
	public static QRCode getMinimumQRCode(String data, int errorCorrectLevel) {
		...
		for (int typeNumber = 1; typeNumber <= 10; typeNumber++) {
			if (length <= QRUtil.getMaxLength(typeNumber, mode,
					errorCorrectLevel)) {
    ...
    public BufferedImage createImage(int cellSize, int margin) throws IOException {
        return createImage(cellSize, margin, null);
    }
	 ...
	 */
	public BufferedImage createImage(int cellSize, int margin, Image logoImage) throws IOException {
		...
		if (logoImage != null) {
            ...
            gs.drawImage(logoImage.getScaledInstance(100, 100, Image.SCALE_SMOOTH), (imageSize - 100) / 2, (imageSize - 100) / 2, null);
            gs.dispose();
        }
	...
	//白色部分采用透明处理的图片
	public BufferedImage createAlphaImage(int cellSize, int margin, Color customColor)
			throws IOException {

		int imageSize = getModuleCount() * cellSize + margin * 2;

		BufferedImage bufImg = new BufferedImage(imageSize, imageSize,
				BufferedImage.TYPE_INT_RGB);

		Graphics2D gs = bufImg.createGraphics();
		bufImg = gs.getDeviceConfiguration().createCompatibleImage(imageSize,
				imageSize, Transparency.TRANSLUCENT);
		gs.dispose();
		gs = bufImg.createGraphics();
		if (customColor == null) {
			gs.setColor(color);
		} else {
			gs.setColor(customColor);
		}
		...
		if (logo != null && logo.exists()) {
			InputStream fis = new FileInputStream(logo);
			Image src_biao = ImageIO.read(fis);
			int wideth_biao = src_biao.getWidth(null);
			int height_biao = src_biao.getHeight(null);
			gs.drawImage(src_biao, (imageSize - wideth_biao) / 2,
					(imageSize - height_biao) / 2, wideth_biao, height_biao, null);
		}
		gs.dispose();
		bufImg.flush();
		return bufImg;
